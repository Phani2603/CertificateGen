...
interface CorporateProfileOverlayProps {
  trigger: ReactNode
}
...
export function CorporateProfileOverlay({ trigger }: CorporateProfileOverlayProps) {
  ...
  const [banner, setBanner] = useState<{ from: string; to: string }>(() => {
    ...
    try {
      const stored = localStorage.getItem("corp-banner-default")
      if (stored) {
  ...
  useEffect(() => {
    ...
    const defaultStored = localStorage.getItem("corp-banner-default")
    if (defaultStored) {
      try {
  ...
  useEffect(() => {
    ...
    try {
      ...
      localStorage.setItem("corp-banner-default", JSON.stringify(banner))
    } catch (e) {
