...
export async function GET(request: NextRequest) {
  try {
    ...
    const certificates = await Certificate.find({
      ...
      .select(
        'verificationId recipientName recipientEmail eventName issueDate templateS3Key fieldConfiguration resolvedFieldValues eventId organizationName clubName organizationId privateOrgId watermarkEnabledAtIssue'
      )
      .lean()
